...
void BackupJob::inspect(sc_save_backup_snapshot& out) const {
    std::lock_guard<std::mutex> guard(mutex_);
    const auto& p = progress_;
    constexpr uint32_t states[] = {SC_BACKUP_WAITING_NATIVE, SC_BACKUP_COPYING, SC_BACKUP_COPIED, SC_BACKUP_COMPLETE, SC_BACKUP_FAILED};
    out.state = states[static_cast<unsigned>(p.state)];
    out.failure = static_cast<uint32_t>(p.failure);
    out.operation_id = p.operation;
    out.flags |= (cancel_.load(std::memory_order_acquire) ? SC_BACKUP_CANCEL_REQUESTED : 0u) |
        (p.readback_terminal ? SC_BACKUP_READ_TERMINAL : 0u) | (p.readback_success ? SC_BACKUP_READ_SUCCESS : 0u) |
        (p.storage_attempted ? SC_BACKUP_STORAGE_ATTEMPTED : 0u) | (p.storage_complete ? SC_BACKUP_STORAGE_COMPLETE : 0u);
    out.storage_outcome = static_cast<uint32_t>(p.storage_result.outcome);
    out.storage_error = p.storage_result.win32_error;
    out.files = p.output.files; out.bytes = p.output.bytes;
    const auto slash = p.output.path.find_last_of(L"\\/");
    const auto start = slash == std::wstring::npos ? 0 : slash + 1;
    // This basename is generated by storage, independently of native file names.
    size_t offset = 0;
    for (size_t i = start; i < p.output.path.size(); ++i) out.basename[offset++] = static_cast<char>(p.output.path[i]);
    out.basename[offset] = 0;
}
